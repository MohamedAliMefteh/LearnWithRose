...
async function attemptAuthentication(body: any, retryCount = 0): Promise<Response> {
  const maxRetries = 2;

  try {
    const response = await fetch(`${EXTERNAL_API_BASE_URL}/api/v1/auth/authenticate`, {
      method: 'POST',
      headers: {
    ...
    if (response.status === 500 && retryCount < maxRetries) {
      console.log(`Authentication failed with 500, retrying... (attempt ${retryCount + 1}/${maxRetries})`);
    ...
    if (retryCount < maxRetries) {
      console.log(`Network error during authentication, retrying... (attempt ${retryCount + 1}/${maxRetries})`);
      await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second
      return attemptAuthentication(body, retryCount + 1);
    }
    throw error;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        errorMessage = 'Backend service is experiencing issues. Please try again later.';
