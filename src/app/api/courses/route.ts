...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        "- Auth header sent:",
        headers["Authorization"] ? "Yes (Bearer token)" : "No",
      );
        ...
        return NextResponse.json(
          {
            ...
            details: errorMessage,
            suggestion:
              "Please check that your account has the necessary permissions to create courses",
          },
          { status: 403 },
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details:
              "The backend service is experiencing issues. Please try again later.",
